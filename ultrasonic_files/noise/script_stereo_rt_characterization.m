...
% stereo monitoring algorithm.  We look at an exactly represented
...
% (stereo results match the mono case)
%
% +---------+----------+----------+
% |  A (V)  | Min (dB) | Max (dB) |
% +---------+----------+----------+
...
% system parameters
nc = 2;                         % number of channels (1 for ultrasound)
Fs = 100E3/nc;
nhw = 12;
ndw = 960;
nfw = nhw+ndw;
...
for ii = 1:num_iters
    ...
    if(ib_idx+nfft > nib)
        ...
        temp = 20*log10(abs(fft(ib(ib_idx+(1:nfft)).*w)));
    end
